...
func (t *IndexTool) Description() string {
	return "Build or query a code index of symbols (functions, types, variables) for fast semantic lookup"
}
...
func (t *IndexTool) InputSchema() map[string]any {
	return map[string]any{
		"type": "object",
		"properties": map[string]any{
			"action":      map[string]any{"type": "string", "description": "Action: build, stats, search, symbols, context", "default": "build"},
			"path":        map[string]any{"type": "string", "description": "Directory to index or file for context"},
			"query":       map[string]any{"type": "string", "description": "Search query (for search/symbols actions)"},
			"type":        map[string]any{"type": "string", "description": "Symbol type filter (function, struct, interface, etc)"},
			"language":    map[string]any{"type": "string", "description": "Language filter"},
			"max_results": map[string]any{"type": "integer", "description": "Maximum results for search", "default": 20},
		},
	}
}
...
func (t *IndexTool) Execute(ctx context.Context, input map[string]any) (any, error) {
	...
	switch action {
	case "build":
		return t.buildIndex(path)
	case "stats":
		return t.getStats(path)
	case "search":
		return t.searchIndex(ctx, input, path)
	case "symbols":
		return t.searchSymbols(ctx, input, path)
	case "context":
		return t.getFileContext(ctx, input, path)
	default:
		return nil, fmt.Errorf("unknown action: %s (valid: build, stats, search, symbols, context)", action)
	}
}

func (t *IndexTool) buildIndex(path string) (any, error) {
	idx := index.NewCodebaseIndex(path)
	if err := idx.Index(); err != nil {
		return nil, &Error{Code: "INDEX_ERROR", Message: err.Error()}
	}

	indexPath := index.GetIndexPath(path)
	if err := idx.Save(indexPath); err != nil {
		logger.Debug("index: failed to save index: %v", err)
	}
...
func (t *IndexTool) getStats(path string) (any, error) {
	idx := getOrCreateIndex(path)
	return map[string]any{
		"path":  path,
		"stats": idx.GetStats(),
	}, nil
...
func (t *IndexTool) searchIndex(ctx context.Context, input map[string]any, basePath string) (any, error) {
	...
	result, err := idx.Search(ctx, index.SearchQuery{
		Query:      query,
		Type:       codeType,
		Language:   language,
...
func (t *IndexTool) searchSymbols(ctx context.Context, input map[string]any, basePath string) (any, error) {
	query, _ := input["query"].(string)
	if query == "" {
		return nil, ErrRequiredField("query")
	}
	...
	symbols, err := idx.SearchSymbols(ctx, query, kind, limit)
	if err != nil {
		return nil, &Error{Code: "SEARCH_ERROR", Message: err.Error()}
	}
...
func (t *IndexTool) getFileContext(ctx context.Context, input map[string]any, basePath string) (any, error) {
	...
	maxTokens := 4000
	if mt, ok := input["max_results"].(int); ok && mt > 0 {
		maxTokens = mt
	}
	...
	context := idx.GetFileContext(ctx, filePath, maxTokens)
	return map[string]any{
		"file":    filePath,
		"context": context,
	}, nil
